...
public class CollisionVisualizer : MonoBehaviour
{
    ...
    private void Update()
    {
        UpdateBoxes(fighter.pushbox, fighter.hurtboxes, fighter.hitboxes);
    }

    public void UpdateBoxes(Pushbox pushbox, List<Hurtbox> hurtboxes, List<Hitbox> hitboxes)
    {
        // --- Update Pushbox ---
        if (pushbox != null && pushboxGO != null)
        {
            UpdateSprite(pushboxGO, pushbox.rect.position, pushbox.rect.size);
        ...
        for (int i = 0; i < hurtboxGOs.Count; i++)
        {
            ...
            {
                UpdateSprite(hurtboxGOs[i], hurtboxes[i].rect.position, hurtboxes[i].rect.size);
        ...
        for (int i = 0; i < hitboxGOs.Count; i++)
        {
            if (i < hitboxes.Count)
            {
                UpdateSprite(hitboxGOs[i], hitboxes[i].rect.position, hitboxes[i].rect.size);
    ...
    private void UpdateSprite(GameObject boxGO, Vector2 position, Vector2 size)
    {
